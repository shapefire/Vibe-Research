...
import { formatBadChains } from "@/lib/source-health";
...
export function SourceStatusBanner() {
  ...
  const degraded = status && Object.values(status.chains).some(
    (c) => c === "degraded" || c === "down",
  );
  if (!degraded || !status) return null;

  const badChains = formatBadChains(status.chains);
  ...
  return (
    <div className="mb-4 flex items-center gap-2 rounded-lg border border-warning/30 bg-warning/5 px-4 py-3 text-sm text-muted-foreground">
      ...
      <span>
        部分数据源异常{badChains ? `（${badChains}）` : ""}，数据可能滞后。
        <Link to="/settings#data-sources" className="ml-1 text-primary hover:underline">检查依赖</Link>
      </span>
    </div>
  );
}
